...
SimpleObject3D::SimpleObject3D(): m_indexBuffer(QOpenGLBuffer::IndexBuffer), m_texture(0) {
    m_scale = 1.0f;
}

SimpleObject3D::SimpleObject3D(const QVector<VertexData> &vertData, const QVector<GLuint> &indexes, const QImage &texture): m_indexBuffer(QOpenGLBuffer::IndexBuffer), m_texture(0) {
    m_scale = 1.0f;
...
SimpleObject3D::~SimpleObject3D() {
    ...
    if ((m_texture != 0) && (m_texture->isCreated())) m_texture->destroy();
}
...
void SimpleObject3D::draw(QOpenGLShaderProgram *program, QOpenGLFunctions *functions) {
    ...
    program->setAttributeBuffer(texLoc, GL_FLOAT, offset, 3, sizeof(VertexData));
